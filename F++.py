...
def printcode(code):
    ...
    for i in range(len(code)):
        if code[i] in number_list:
            try:
                if not (
                    (code[i + 1] == "m" or code[i + 2] == "m")
                    or (code[i + 1] == ";" or code[i + 2] == ";")
                ):
                    new_code = new_code + NUMBER_COLOR + code[i] + END_COLOR
                else:
    ...
    code = code.replace("pr\033[94min\033[0mt",
                        FUNCTION_COLOR + "print" + END_COLOR)
    code = code.replace("\033[94min\033[0put",
                        FUNCTION_COLOR + "input" + END_COLOR)
    code = code.replace("__init__", BUILT_FUNCTION + "__init__" + END_COLOR)
    code = code.replace(
        "__\033[94min\033[0mit__", BUILT_FUNCTION + "__init__" + END_COLOR
    )
        ...
        if i[0:-1] in vas:
            if type(vas[i[0:-1]]) == str:
                if "." in vas[i[0:-1]] or "e" in vas[i[0:-1]]:
                    try:
                        le.append(math.factorial(float(vas[i[0:-1]])))
                    except:
                        le.append(math.factorial(vas[i[0:-1]]))
                else:
                    try:
                        le.append(math.factorial(int(vas[i[0:-1]])))
                    except:
                        le.append(math.factorial(vas[i[0:-1]]))
            else:
                le.append(math.factorial(vas[i[0:-1]]))
            ...
            if type(i[0:-1]) == str:
                if "." in i[0:-1] or "e" in i[0:-1]:
                    try:
                        le.append(math.factorial(float(i[0:-1])))
                    except:
                        le.append(math.factorial(i[0:-1]))
                else:
                    try:
                        le.append(math.factorial(int(i[0:-1])))
                    except:
                        le.append(math.factorial(i[0:-1]))
            else:
                le.append(math.factorial(i[0:-1]))
...
class Error:
    def output_error1(self, line):
        try:
            raise ValueError(
                f"Output Error:Output in the wrong place in line {line}")
    ...
    def output_error2(self, line):
        try:
            raise ValueError(
                f"Output Error:var has not defined in line {line}")
        except ValueError as e:
            print(repr(e))
    ...
    def var_error1(self, line):
        try:
            raise ValueError(
                f"Var Error:value name or content has not been found in line {line}"
            )
        except ValueError as e:
            print(repr(e))
    ...
    def var_error2(self, line):
        try:
            raise ValueError(f"Var Error:bad var sentence in line {line}")
        except ValueError as e:
            print(repr(e))

    def entry_error1(self, line):
        try:
            raise ValueError(f"Entry Error:bad entry sentence in line {line}")
        except ValueError as e:
            print(repr(e))

    def note_error1(self, line):
        try:
            raise ValueError(f"Note Error:// is in the wrong place {line}")
        except ValueError as e:
            print(repr(e))
...
class FPlusPlusParse:
        ...
        if code[0] == "fout":
            ...
            for i in range(len(code)):
                if code[i][0] == '"':
                    ...
                    if code[i] in self.var_name:
                        print(self.var_value[self.var_name.index(code[i])])
                    else:
                        try:
                            print(lam(code[i]))
                        except:
                            _error.output_error2(a)
        else:
            _error.output_error1(a)
    ...
    def var(self, code, i):
        ...
        try:
            ...
            try:
                self.var_value.append(lam(code[1]))
            except:
                self.var_value.append(code[1])
        except:
            _error.var_error2(i)
    ...
    def entry(self, code, i):
        ...
        if len(code) == 3:
            if code[2][-1] == '"':
                ...
                code2 = code[2].split("<-")
                if code2[1] in self.var_name:
                    a = input(self.var_value[self.var_name.index(code2[1])])
                else:
                    try:
                        a = input(lam(code2[1]))
                    except:
                        _error.entry_error1(i)
            if code[0] in self.var_name:
                number = self.var_name.index(code[0])
                del self.var_name[number]
                del self.var_value[number]
    ...
    def note(self, code, i):
        if code[0:2] != "//":
            _error.note_error1(i)


_F_plus_plus_parse = FPlusPlusParse()
...
class FPlusPlus:
    ...
    def run_code(self):
        ...
        for i in range(len(self.code)):
            if "fout" in self.code[i]:
                _F_plus_plus_parse.output(self.code[i], i)
            elif "var" in self.code[i]:
                _F_plus_plus_parse.var(self.code[i], i)
            elif "entry" in self.code[i]:
                _F_plus_plus_parse.entry(self.code[i], i)
            elif "//" in self.code[i]:
                _F_plus_plus_parse.note(self.code[i], i)


print(
    """欢迎使用F++ Stable 200(Rebuild重构版)
...
_F_plus_plus = FPlusPlus(code_list)
_F_plus_plus.run_code()
print("\033[33m编译结束")
